...
final class AddTransactionViewController: UIViewController {
    ...
    // Amount card — contains currency button + dropdown + amount field
    private let amountCard       = UIView()
    private let currencyButton   = UIButton(type: .system)
    private let currencyDropdown = UIView()
    private var dropdownRows: [UIButton] = []

    // Amount field shown below dropdown (or below button when closed)
    private let amountTextField = UITextField()
    private let convertedLabel  = UILabel()
    ...
    // Dropdown height constraint (animatable)
    private var dropdownHeightConstraint: NSLayoutConstraint!
    // Amount field hidden constraint when dropdown open
    private var amountTopWhenClosed: NSLayoutConstraint!
    private var amountTopWhenOpen:   NSLayoutConstraint!
    ...
    // MARK: - Setup: Amount Card
    // Layout inside amountCard (top → bottom):
    //   [currencyButton]
    //   [currencyDropdown]   ← height=0 when closed, expands when open
    //   [amountTextField]
    //   [convertedLabel]
    ...
    private func setupAmountCard() {
        amountCard.backgroundColor    = UIColor.secondarySystemGroupedBackground
        amountCard.layer.cornerRadius = 24
        amountCard.clipsToBounds      = true   // dropdown clips inside card
        amountCard.translatesAutoresizingMaskIntoConstraints = false
        ...
        // ── Currency Dropdown ────────────────────────────────────────
        currencyDropdown.backgroundColor    = UIColor.tertiarySystemGroupedBackground
        currencyDropdown.layer.cornerRadius = 0
        currencyDropdown.clipsToBounds      = true
        currencyDropdown.translatesAutoresizingMaskIntoConstraints = false
        ...
        buildDropdownRows(into: dropdownStack)
        ...
        [currencyButton, currencyDropdown, amountTextField, convertedLabel]
            .forEach { amountCard.addSubview($0) }

        contentView.addSubview(amountCard)
    }

    // MARK: - Build Dropdown Rows

    private func buildDropdownRows(into stack: UIStackView) {
    ...
    private func makeCurrencyRow(currency: CurrencyRate, index: Int) -> UIButton {
        let btn = UIButton(type: .system)
        btn.tag = index
        btn.addTarget(self, action: #selector(currencySelected(_:)), for: .touchUpInside)
        btn.translatesAutoresizingMaskIntoConstraints = false
        ...
        if currency.code == "UZS" {
            ...
            rateLabel.text = "1 = \(formatSum(currency.rateToUZS)) so'm"
            rateLabel.textColor = .secondaryLabel
        }
        rateLabel.font = .systemFont(ofSize: 12, weight: .regular)
        rateLabel.translatesAutoresizingMaskIntoConstraints = false
        rateLabel.isUserInteractionEnabled = false

        // Checkmark for selected currency
        let checkView = UIImageView()
        checkView.image = UIImage(systemName: "checkmark",
                                  withConfiguration: UIImage.SymbolConfiguration(pointSize: 12, weight: .semibold))
        checkView.tintColor = accentTeal
        checkView.translatesAutoresizingMaskIntoConstraints = false
        checkView.isHidden = (currency.code != selectedCurrency.code)
        checkView.isUserInteractionEnabled = false
        checkView.tag = 999  // tag to find it later

        [flagLabel, textStack, rateLabel, checkView].forEach { btn.addSubview($0) }
        ...
        NSLayoutConstraint.activate([
            ...
            checkView.trailingAnchor.constraint(equalTo: btn.trailingAnchor, constant: -16),
            checkView.centerYAnchor.constraint(equalTo: btn.centerYAnchor),
            checkView.widthAnchor.constraint(equalToConstant: 16),
            checkView.heightAnchor.constraint(equalToConstant: 16),

            rateLabel.trailingAnchor.constraint(equalTo: checkView.leadingAnchor, constant: -8),
            rateLabel.centerYAnchor.constraint(equalTo: btn.centerYAnchor),
        ])
    ...
    // MARK: - Currency Button appearance

    private func updateCurrencyButton() {
        let symbol = currencySymbol(for: selectedCurrency.code)
        let title  = "\(selectedCurrency.flag)  \(selectedCurrency.code)"
        let chevron = UIImage(systemName: isCurrencyDropdownOpen ? "chevron.up" : "chevron.down",
                              withConfiguration: UIImage.SymbolConfiguration(pointSize: 10, weight: .semibold))
        var config = UIButton.Configuration.plain()
        config.title = title
    ...
    private func makeCategoryButton(cat: (name: String, icon: String, systemIcon: String),
                                    index: Int) -> UIButton {
        let btn = UIButton(type: .custom)
        btn.tag = index
        btn.addTarget(self, action: #selector(categoryTapped(_:)), for: .touchUpInside)
        ...
        let emojiLabel = UILabel()
        emojiLabel.text = cat.icon; emojiLabel.font = .systemFont(ofSize: 24)
        emojiLabel.textAlignment = .center
        emojiLabel.translatesAutoresizingMaskIntoConstraints = false

        let nameLabel = UILabel()
        nameLabel.text = cat.name; nameLabel.font = .systemFont(ofSize: 11, weight: .medium)
        nameLabel.textColor = .secondaryLabel; nameLabel.textAlignment = .center
        nameLabel.numberOfLines = 2
        nameLabel.translatesAutoresizingMaskIntoConstraints = false

        btn.addSubview(emojiLabel); btn.addSubview(nameLabel)
        NSLayoutConstraint.activate([
            emojiLabel.topAnchor.constraint(equalTo: btn.topAnchor, constant: 12),
            emojiLabel.centerXAnchor.constraint(equalTo: btn.centerXAnchor),
            nameLabel.topAnchor.constraint(equalTo: emojiLabel.bottomAnchor, constant: 4),
    ...
    private func activateConstraints() {
        ...
        let totalDropdownH = CGFloat(currencies.count) * rowH
                           + CGFloat(currencies.count - 1) * sepH
        ...
        NSLayoutConstraint.activate([
            ...
            // Currency button at top of card
            currencyButton.topAnchor.constraint(equalTo: amountCard.topAnchor, constant: 16),
            currencyButton.centerXAnchor.constraint(equalTo: amountCard.centerXAnchor),
            currencyButton.heightAnchor.constraint(equalToConstant: 34),

            // Dropdown right below currency button (no gap)
            currencyDropdown.topAnchor.constraint(equalTo: currencyButton.bottomAnchor, constant: 8),
            currencyDropdown.leadingAnchor.constraint(equalTo: amountCard.leadingAnchor),
            currencyDropdown.trailingAnchor.constraint(equalTo: amountCard.trailingAnchor),
            dropdownHeightConstraint,

            // Amount field below dropdown
            amountTextField.topAnchor.constraint(equalTo: currencyDropdown.bottomAnchor, constant: 8),
            amountTextField.leadingAnchor.constraint(equalTo: amountCard.leadingAnchor, constant: 16),
            amountTextField.trailingAnchor.constraint(equalTo: amountCard.trailingAnchor, constant: -16),
            amountTextField.heightAnchor.constraint(equalToConstant: 64),

            // Converted label below amount
            convertedLabel.topAnchor.constraint(equalTo: amountTextField.bottomAnchor, constant: 2),
            convertedLabel.leadingAnchor.constraint(equalTo: amountCard.leadingAnchor, constant: 16),
            convertedLabel.trailingAnchor.constraint(equalTo: amountCard.trailingAnchor, constant: -16),
            convertedLabel.bottomAnchor.constraint(equalTo: amountCard.bottomAnchor, constant: -16),
            ...
            categoryStack.topAnchor.constraint(equalTo: sectionLabel.bottomAnchor, constant: 8),
            categoryStack.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: pad),
            categoryStack.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -pad),
    ...
    @objc private func toggleCurrencyDropdown() {
        ...
        // When opening: hide keyboard so user must pick currency first
        if isCurrencyDropdownOpen {
        ...
        let rowH: CGFloat = 56
        let sepH: CGFloat = 0.5
        let totalH = CGFloat(currencies.count) * rowH
                   + CGFloat(currencies.count - 1) * sepH

        UIView.animate(withDuration: 0.38, delay: 0,
                       usingSpringWithDamping: 0.85, initialSpringVelocity: 0.2) {
            self.dropdownHeightConstraint.constant = self.isCurrencyDropdownOpen ? totalH : 0
            self.amountTextField.alpha  = self.isCurrencyDropdownOpen ? 0 : 1
            self.convertedLabel.alpha   = self.isCurrencyDropdownOpen ? 0 : 1
            self.view.layoutIfNeeded()
        }
    }

    @objc private func currencySelected(_ sender: UIButton) {
        let chosen = currencies[sender.tag]
        selectedCurrency = chosen

        // Update checkmarks in dropdown
        for row in dropdownRows {
            if let check = row.subviews.first(where: { $0.tag == 999 }) as? UIImageView {
                check.isHidden = (row.tag != sender.tag)
            }
        }

        // Update currency button with new flag + code
        updateCurrencyButton()
        ...
        UIView.animate(withDuration: 0.35, delay: 0,
            ...
            self.convertedLabel.alpha  = 1
            self.view.layoutIfNeeded()
        } completion: { _ in
            self.amountTextField.becomeFirstResponder()
        }

        // Refresh conversion label
        amountChanged()
    }
    ...
    @objc private func saveTapped() {
        guard let titleText = nameField.text, !titleText.isEmpty,
              let amountText = amountTextField.text, !amountText.isEmpty else {
            shakeInvalidFields(); return
        }

        let cleaned = amountText
            .replacingOccurrences(of: " ", with: "")
            .replacingOccurrences(of: ",", with: ".")
        guard let raw = Double(cleaned), raw > 0 else {
            shakeInvalidFields(); return
        }

        // Convert to UZS
        let amountInUZS = raw * selectedCurrency.rateToUZS
        ...
        viewModel.save(
            ...
            if let error = error { print("☁️ Xato: \(error.localizedDescription)") }
        }
    ...
    private func shakeInvalidFields() {
        UINotificationFeedbackGenerator().notificationOccurred(.error)
        let fields: [UIView] = [amountTextField, nameField].filter {
            ($0 as? UITextField)?.text?.isEmpty ?? false
        }
        for field in fields {
            let anim = CAKeyframeAnimation(keyPath: "transform.translation.x")
            anim.timingFunction = CAMediaTimingFunction(name: .linear)
...
extension AddTransactionViewController {
    ...
    func loadLiveCurrencies() {
        CurrencyService.shared.fetchRates { [weak self] liveCurrencies in
            ...
            let previousCode = self.selectedCurrency.code
            self.currencies = liveCurrencies
            self.selectedCurrency = liveCurrencies.first(where: { $0.code == previousCode })
                ?? liveCurrencies.first
                ?? CurrencyService.shared.fallbackRates()[0]

            // Rebuild dropdown rows with new rates
            if let stack = self.currencyDropdown.subviews.first as? UIStackView {
                self.buildDropdownRows(into: stack)
            }
            self.amountChanged()
            self.updateCurrencyButton()
        }
